...
public class LayeredMapping : MonoBehaviour {
    ...
    public static float Zoffset = 21f;
    ...
    public static void hideOthers(int num)
    {
        if(layers != null)
        {
    ...
    public static void addDatatoMaps() {
        for(int i = 0; i < SmokeyController.instantiators.Count; i++)
        {
            ...
            foreach (Transform dp in SmokeyController.instantiators[i].dataPoints)
            {
                if (dateNode != null)
                {
                    ...
                    dataTrf.gameObject.SetActive(true);
                    dataTrf.GetComponent<PrefabID>().id = nodeSize;
                    dataTrf.name = "3,2," + SmokeyController.logLayerCount.ToString();
                    nodeSize++;
                    SmokeyController.logLayerCount++;
